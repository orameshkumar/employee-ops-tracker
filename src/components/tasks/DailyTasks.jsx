...
const s = {
  ...
  saving: { color: '#94a3b8', fontSize: '0.8rem', marginTop: 6 },
...
export default function DailyTasks() {
  ...
  useEffect(() => {
    getTodayDailyTasks(user.uid).then(setSaved)
  }, [user.uid])

  const savedNames = saved.map(t => t.taskName)
  const pct = Math.round((savedNames.length / DEFAULT_TASKS.length) * 100)
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={{ color: '#94a3b8', fontSize: '0.8rem', marginBottom: 16 }}>{savedNames.length} / {DEFAULT_TASKS.length} tasks completed ({pct}%)</div>
